...
def likelihood_func(w, X, y_train, likelihood_var):
    ...
    for i in range(n):
        factor = 1/(np.sqrt(2*np.pi*likelihood_var))
        exponent = -((y_train[i]-X[i,:]@w).T@(y_train[i]-X[i,:]@w))/(2*likelihood_var)
        likelihood_i = factor*np.exp(exponent)
...
def get_posterior_params(X, y_train, prior, likelihood_var = 0.2**2):
    ...
    post_mean = np.linalg.inv(X.T@X + likelihood_var*np.linalg.inv(sigma))@X.T@y_train
                
    post_var = np.linalg.inv(likelihood_var*X.T@X + np.linalg.inv(sigma))
